...
- **`createWorker({ timeoutMs })`**: default is now **60000 ms** (1 minute). Set **`timeoutMs: 0`** for no execution timeout (previous default behavior).

### Added

- **`DEFAULT_WORKER_TIMEOUT_MS`** export (same numeric default)
